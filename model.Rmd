...
ggplot(residuals_long, aes(x = Model, y = Residual)) +
  ...
  theme_minimal() +
  labs(title = "Residuals for each Model using LOOCV")
```
...
```{r}
residuals_list <- list(
  "Temporal Discounting" = rep(NA, nrow(sp_data_train)),
  "Agency" = rep(NA, nrow(sp_data_train)),
  "Loss Aversion" = rep(NA, nrow(sp_data_train))
)

train_coords <- coordinates(sp_data_train)

# Temporal Discounting LOOCV
for (i in 1:nrow(sp_data_train)) {
...
```{r}
residuals_df <- data.frame(
  Observation = 1:nrow(sp_data_train),
  Temporal_Discounting = residuals_list[["Temporal Discounting"]],
  Agency = residuals_list[["Agency"]],
  Loss_Aversion = residuals_list[["Loss Aversion"]]
)
